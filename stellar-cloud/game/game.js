(() => {
  ...
  let STORAGE_KEY = "se_cloud_preview_v1";
  const ROSTER_PREFIX = "se_cloud_preview_roster_v1:";
  const VERSION = 1;
  ...
  function setScreen(html) {
    ...
    window.mountCourse(elScreen, state, (route) => {
      if (!state.starName) state.starName = "课堂示范星";
  ...
  function ensureUnlocked(level, nextRoute) {
    return pushRoute(nextRoute);
  }

  function showTeacherPanel() {
    teacherCodesEl.textContent = "课件模式：所有章节无需密码。";
  ...
  function hardResetAll() {
    clearMinigame();
    Object.keys(localStorage).filter((key) => key === STORAGE_KEY || key.startsWith(ROSTER_PREFIX)).forEach((key) => localStorage.removeItem(key));
    state = loadState();
    historyStack.length = 0;
    render();
  }
  ...
  function renderL4() {
    ...
    setScreen(`
      <div class="col">
        ...
        <div class="p muted">请从外到内填写简化洋葱结构各层的元素，最后一层是铁核心：</div>
  ...
  function renderL5() {
    ...
    $("#btnRegistry")?.addEventListener("click", () => window.parent.postMessage({type:"stellar-register"}, location.origin));
  }
  ...
  function renderBrownDwarfEnd() {
    setScreen(`
      <div class="col">
        ...
        <button class="btn" id="btnRestart2">返回星区登记寄语</button>
      </div>
    `);
    $("#btnRestart2")?.addEventListener("click", () => window.parent.postMessage({type:"stellar-register"}, location.origin));
  }
  ...
  if (window.parent === window) {
    setScreen('<p>请从云端星区页面打开课件。</p>');
  } else {
    btnTeacher.hidden = true;
    btnRestart.hidden = true;
    window.addEventListener("message", (event) => {
      if (event.origin !== location.origin || event.source !== window.parent || event.data?.type !== "stellar-init") return;
      const incoming = event.data;
      if (typeof incoming.storageKey !== "string" || !incoming.storageKey.startsWith("stellar-cloud-progress:")) return;
      STORAGE_KEY = incoming.storageKey;
      state = loadState();
      if (!state.l1Rolled) {
        state.starName = incoming.star.star_name;
        state.galaxyName = incoming.classroom;
        state.mass = Number(incoming.star.initial_mass);
        state.l1Rolled = true;
        state.route = "l1";
      }
      saveState();
    ...
    window.parent.postMessage({type:"stellar-ready"}, location.origin);
  }
})();
